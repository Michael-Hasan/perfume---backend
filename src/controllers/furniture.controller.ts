...
furnitureController.getFurniture = async (
  req: { params: { id: any }; member: { _id: null } },
  res: {
    status: (arg0: number) => {
      (): any;
      new (): any;
      json: { (arg0: Furniture): void; new (): any };
    };
  }
) => {
  try {
...
furnitureController.getRandomFurnitures = async (
  req: Request,
  res: Response
) => {
  try {
    const result = await furnitureService.getRandomFurnitures();
    res.status(HttpCode.OK).json({ furnitures: result });
  } catch (err) {
...
furnitureController.getComingSoonFurnitures = async (
  req: Request,
  res: Response
) => {
  try {
    const furnitures = await furnitureService.getComingSoonFurnitures(4);
...
furnitureController.createNewProduct = async (
  req: AdminRequest,
  res: Response
) => {
  try {
    ...
    res.send(
      `<script> alert ("Succesfully created"); window.location.replace('/admin/furniture/all') </script>`
    );
...
furnitureController.updateChosenFurniture = async (
  req: Request,
  res: Response
) => {
  try {
